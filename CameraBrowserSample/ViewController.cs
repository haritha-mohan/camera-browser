...
namespace CameraBrowserSample
{
    ...
    public partial class ViewController : NSViewController, IICDeviceBrowserDelegate, IICCameraDeviceDelegate, IICCameraDeviceDownloadDelegate, IICDeviceDelegate {
        ...
        public override void ViewDidLoad () //application did finish launching
		{
            ...
            //CameraContentTableView.FindTableColumn((NSString)"Date").Bind((NSString)"value", MediaFilesController, "arrangedObjects.metadataIfAvailable.{Exif}.DateTimeOriginal", null);
            //CameraContentTableView.FindTableColumn((NSString)"Make").Bind((NSString)"value", MediaFilesController, "arrangedObjects.metadataIfAvailable.{TIFF}.Make", null);
            //CameraContentTableView.FindTableColumn((NSString)"Model").Bind((NSString)"value", MediaFilesController, "arrangedObjects.metadataIfAvailable.{TIFF}.Model", null);
        ...
        public void downloadFiles (ICCameraFile[] files)
        {
            ...
            foreach (ICCameraFile f in files)
            {
                ...
                f.Device.RequestDownloadFile(f, dict, didDownloadDataDel);
            }
        }

        public void didDownloadDataDel(ICCameraFile f, NSError err, NSDictionary options)
        {
        ...
        public void didDownloadFile (ICCameraFile f, NSError err, NSDictionary options, IntPtr contextInfo)
        {
            Console.WriteLine("didDownloadFile called with:");
            Console.WriteLine($"file: {f}");
            Console.WriteLine($"error: {err}");
            Console.WriteLine($"options: {options}");
            Console.WriteLine($"contextInfo: {contextInfo}");
        ...
        [Export("didReadData:fromFile:error:contextInfo:")]
        public void didReadData(NSData data, ICCameraFile file, NSError err, IntPtr contextInfo)
        {
            Console.WriteLine("didReadData called with:");
            Console.WriteLine($"data: {data}");
            Console.WriteLine($"file: {file}");
